...
int DXScreenCapture::init(int displayIndex)
{
	if (m_initialized)
	{
		return -1;
...
int DXScreenCapture::createSharedTexture()
{
...
int DXScreenCapture::exit()
{
	return 0;
}

int DXScreenCapture::start()
{
	if (!m_initialized)
	{
		return -1;
...
int DXScreenCapture::aquireFrame()
{
	Microsoft::WRL::ComPtr<IDXGIResource> dxgiResource;
...
int DXScreenCapture::captureFrame(std::shared_ptr<uint8_t>& bgraPtr, uint32_t& size)
{
	std::lock_guard<std::mutex> locker(m_mutex);
...
int DXScreenCapture::getTextureHandle(HANDLE* handle, int* lockKey, int* unlockKey)
{
...
int DXScreenCapture::captureImage(std::string pathname)
{
	std::lock_guard<std::mutex> locker(m_mutex);
